...
import StateDescriptors from '../state-descriptors.js';
...
let startup = function () {
    ...
    Meteor.publish('settings', function (searchCriteria = {}, options = {}) {
        //log.debug('publishing setting groups');
        if (options.fields) {
        ...
        if (!this.userId) {
            // if the users not logged in they are restricted to public settings groups
            searchCriteria = {
                $and: [
    ...
    /**
     * Offer Settings Descriptors collection
     */

    Meteor.publish('settings_descriptors', function (searchCriteria = {}, options = {}) {
        //log.debug('publishing ssettings descriptors');
        return SettingsDescriptors.find({});
    });
    SettingsDescriptors.allow(allowAll);
    SettingsDescriptors.deny(denyAll);
    ...
    Meteor.publish('state', function (searchCriteria = {}, options = {}) {
        //log.debug('publishing state groups');
        if (options.fields) {
            options.fields._id = 1;
            options.fields._groupName = 1;
            options.fields._groupIndex = 1;
        ...
        if (!this.userId) {
            ...
            searchCriteria = {
                $and: [
                    {_groupName: {$in: ['system', 'firmware_files']}},
                    searchCriteria
                ]
            };
    ...
    /**
     * Offer State Descriptors collection
     */

    Meteor.publish('state_descriptors', function (searchCriteria = {}, options = {}) {
        //log.debug('publishing state descriptors');
        return StateDescriptors.find({});
    });
    StateDescriptors.allow(allowAll);
    StateDescriptors.deny(denyAll);
    ...
    if (Meteor.users.find().count() == 0) {
        ...
        var uid = Accounts.createUser({
            username: 'admin',
            email: 'joeadmin@digi.com',
            password: 'p@ssw0rd',
            profile: {name: 'joe admin'}
        });
        console.log('joe admin created: ', uid);
        uid = Accounts.createUser({
            username: 'user',
            email: 'joeuser@digi.com',
            password: 'p@ssw0rd',
            profile: {name: 'joe user'}
        });
    ...
    /**
     * Offer Cli messages collection
     */

    Meteor.publish('cli_output', function (searchCriteria = {}, options = {}) {
        console.log('publishing cli messages');
        if (!this.userId) {
            // if the users not logged in we don't publish anything
            return this.ready();
        }
        // return CliOutput.find(searchCriteria, options);
        return CliOutput.find({});
    });
    CliOutput.allow(allowAll);
    CliOutput.deny(denyAll);
...
function parseRciXml(xmldata) {
    ...
    let rciObj = parseStringSync(xmldata, {explicitArray: false, mergeAttrs: true, attrNameProcessors: [name => '@'+name]});
    return rciObj;
}
...
function loadDescriptors(collection, rciObj) {
    var {DeviceMetaData: {dmData: {descriptor: {format_define: {descriptor: descriptorRoot}}}}} = rciObj
    // console.log('loadDescriptors: collection: ', JSON.stringify(descriptorRoot,null, 4));
    if (!!descriptorRoot) {
        descriptorRoot.forEach(descriptor => {
