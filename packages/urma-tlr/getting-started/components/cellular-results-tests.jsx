...
describe('Cellular Results (Wizard) tests', function () {
    ...
    let sandbox, stubState, stubSettings;
    let deviceCtx;
    beforeEach(() => {
    ...
    it('should render cellular results components', function () {
        const root = TestUtils.renderIntoDocument(<CellularResultsParent />);
        const cellularResults = TestUtils.findRenderedComponentWithType(root, CellularResults);
        const cellularSimStatus = TestUtils.findRenderedComponentWithType(root, CellularSimStatus);
        const header = TestUtils.findRenderedDOMComponentWithTag(cellularResults, 'h6');
    ...
    it('verify all the state is being rendered correctly', function () {
        const root = TestUtils.renderIntoDocument(<CellularResultsParent />);
        const cellularResults = TestUtils.findRenderedComponentWithType(root, CellularResults);
        const cellularResultsDisplay = TestUtils.findRenderedDOMComponentWithClass(cellularResults, 'wizard-content-area-right');
        const labels = TestUtils.scryRenderedDOMComponentsWithTag(cellularResults, 'label');
        const spans = TestUtils.scryRenderedDOMComponentsWithTag(cellularResults, 'span');
