        ...
        return (
            <div>
                {this.upToDate() ?
                    ...
                    <div>
                        <form className="box">
                            <label style={{paddingRight: '10px'}} className="bold">Available Version:</label>
                                <select name="firmware versions" onChange={this.props.handleSelect}>
                                    {this.props.developmentMode ?
                                        this.props.fwFiles.map(fwVersion =>
                                            <option key={fwVersion.version} className="firmware-update-option-auto"
                                                value={fwVersion.name}>{fwVersion.version}</option>
                                        ) :
                                        <option className="firmware-update-option-auto"
                                            value={this.findMostRecentVersion(this.props.fwFiles).name}>{this.findMostRecentVersion(this.props.fwFiles).version}</option>
                                    }
                                    <option className="firmware-update-option-manual" value="manual">Choose saved
                                        version
                                    </option>
                                </select>
                        </form>
