...
export default class FirmwareUpdateManual extends Component {
    ...
    getMeteorData() {
        ...
        if (fwStatusSubscription.ready()) {
            ...
            if (fwStatus) {
                // Stringify the status for easy comparison. Only send status to progressMonitor if changed.
                // Avoids resending the same status just because getMeteorData got invoked, which can cause
                // issues in the progress dialog.
                let newFwStatus = JSON.stringify({ s: fwStatus.status, p: fwStatus.progress, r: fwStatus.result });
                if (newFwStatus !== this.lastFwStatus) {
                    this.progressMonitor.update(fwStatus.status, fwStatus.progress, fwStatus.result);
                    this.lastFwStatus = newFwStatus;
                }
            }
        }
    ...
    startFirmwareUpdate(e) {
        ...
        if (this.state.upToDate && !this.selectedFile) {
            ...
            if (this.state.selectedFirmwareVersion !== 'manual') {
                Meteor.call('updateFirmware', this.state.selectedFirmwareVersion, {
                    autoReboot: false,
                    remoteLoad: true
                });
            }
            ...
            return (
                <div>
                    <p className="error"><span className="bold">Error: </span>
                        Could not retrieve available firmware versions.
                    </p>
                </div>
            )
        } else {
