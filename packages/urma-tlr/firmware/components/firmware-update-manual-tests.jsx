...
describe('FirmwareUpdateManual', function () {
    ...
    beforeEach(()=> {
        ...
        stubState.findOne.withArgs({ _groupName: 'system' }).returns({
            _id: 1,
    ...
    it('should not emit duplicate firmware status updates', sinon.test(function () {
        let root = TestUtils.renderIntoDocument(<Parent/>);
        let fwUpdateManual = TestUtils.findRenderedComponentWithType(root, FirmwareUpdateManual);
        ...
        fwUpdateManual.getMeteorData(); // Should emit here
        fwUpdateManual.getMeteorData();
        fwUpdateManual.getMeteorData();
        fwStatus.progress += 1;
        fwUpdateManual.getMeteorData(); // Should emit again here
        fwUpdateManual.getMeteorData();

        sinon.assert.calledTwice(stubFwMgr.onProgress);
    }));
    ...
    it('should set upToDate to true if firmware up to date', sinon.test(function () {
        stubState.findOne.withArgs({ _groupName: 'system' }).returns({
            _id: 1,
            firmware_version: '1.2.3.5 3/18/2016 17:19:03'
        });
