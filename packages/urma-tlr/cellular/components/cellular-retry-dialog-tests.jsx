...
describe('Cellular Retry Dialog (Wizard) tests', function () {
    ...
    it('should render cellular retry dialog components', function () {
        let root = TestUtils.renderIntoDocument(<CelluarRetryDialog onClose={fakeFunc} onSuccess={fakeFunc}
            cellularAPN="" cellularIntState={cellularIntState}
            cellularSettingsId="0"
            deviceSettings={deviceSettings}/>);
        let buttons = TestUtils.scryRenderedDOMComponentsWithTag(root, 'button');
        let cancelBtn = buttons.find((button)=> {
            return button.id == 'cancelBtn'
        });
        let retryBtn = buttons.find((button)=> {
            return button.id == 'retryBtn'
        });
    ...
    it('should call onClose function when cancel button is clicked', sinon.test(function () {
        ...
        let root = TestUtils.renderIntoDocument(<CelluarRetryDialog onClose={spyOnClose} onSuccess={spyonSuccess}
            cellularAPN="" cellularIntState={cellularIntState}
            cellularSettingsId="0"
            deviceSettings={deviceSettings}/>);
        let buttons = TestUtils.scryRenderedDOMComponentsWithTag(root, 'button');
