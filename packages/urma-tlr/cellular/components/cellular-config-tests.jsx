...
let cellular_countries = {
    ...
    "Argentina": [
        ...
        { "provider": "Claro", "username": "", "password": "", "APN": ["igprs.claro.com.ar", "mms.ctimovil.com.ar"] },
        {
            ...
            "username": "",
            "password": "",
            "APN": ["internet.unifon", "mms.gprs.unifon.com.ar", "wap.gprs.unifon.com.ar", "mms.gprs.unifon.com.ar"]
        },
...
describe('Cellular Config (Wizard) tests', function () {
    ...
    it('should render cellular config fields', function () {
        let root = TestUtils.renderIntoDocument(<CellularConfig onVerify={fakeFunc} callBackParent={fakeFunc}
        ...
        let country = selects.find((select)=> {
            return select.name == 'cellular country'
        });
        let apn = selects.find((select)=> {
            return select.name == 'cellular APN'
        });
        let provider = selects.find((select)=> {
            return select.name == 'cellular provider'
        });
        let username = inputs.find((input)=> {
            return input.placeholder == "Username"
        });
        let password = inputs.find((input)=> {
            return input.placeholder == "Password"
        });
        let simPin = inputs.find((input)=> {
            return input.placeholder == "N/A"
        });
        let customApn = inputs.find((input)=> {
            return input.placeholder == "Enter your custom APN here"
        });
    ...
    it('when provider is verizon, apn should have value null and custom and also no advanced options available', function () {
        ...
        let simpin = inputs.find((input)=> {
            return input.id == "cellularSIMPin"
        });

        // Set country,provider,apn other than verizon and verify all the UI components are right
        ...
        expect(simpin).to.be.undefined;

        // set provider value to verizon and verify apn is ""
        ...
        apn = selects.find((select)=> {
            return select.id == 'cellularAPN'
        });
        expect(apn).to.not.be.undefined;
        expect(apn.value).to.equal('');
        expect(apn.disabled).to.be.false;
        expect(customApn).to.be.undefined;

        // set apn value to custom and verify custom apn is defined
        TestUtils.Simulate.change(apn, { target: { id: "cellularAPN", value: 'custom' } });
        inputs = TestUtils.scryRenderedDOMComponentsWithTag(root, 'input');
        customApn = inputs.find((input)=> {
            return input.id == "cellularCustomAPN"
        });
        expect(customApn).to.not.be.undefined;

        // verify that the advanced options are hidden for Verizon
        username = inputs.find((input)=> {
            return input.id == "cellularUsername"
        });
        password = inputs.find((input)=> {
            return input.id == "cellularPassword"
        });
        simpin = inputs.find((input)=> {
            return input.id == "cellularSIMPin"
        });
        expect(username).to.be.undefined;
        expect(password).to.be.undefined;
    ...
    it('show/hide username, password, simpin when advanced is clicked ', function () {
        ...
        let advanced = TestUtils.findRenderedDOMComponentWithClass(root, 'expander-trigger');

        // verify that the advanced options are hidden by default
        expect(username).to.be.undefined;
        expect(password).to.be.undefined;
        expect(simpin).to.be.undefined;

        // click advanced link and verify that the advanced options are shown
        ...
        // click advanced link again and verify that the advanced options are hidden again
        TestUtils.Simulate.click(advanced);
        inputs = TestUtils.scryRenderedDOMComponentsWithTag(root, 'input');
        username = inputs.find((input)=> {
            return input.id == "cellularUsername"
