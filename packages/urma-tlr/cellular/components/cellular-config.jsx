...
export default class CellularConfig extends Component {
    constructor(props) {
        ...
        this.handleAPNSelect = this.handleAPNSelect.bind(this);
        this.handleAdvancedOptions = this.handleAdvancedOptions.bind(this);
    }
    ...
    toggle() {
        ...
        /** when hiding advanced options clear username and password in parent **/
        if (this.state.showAdvanced) {
            this.props.callBackParent('resetAdvanced', '');
        }
    }
    ...
    callBackParent() {
        ...
        if (this.state.selectedProvider === 0) {
            ...
            this.setProviderVerizon(provider);
        }
        if (this.state.selectedAPN === 0) {
            ...
            this.props.callBackParent('cellularAPN', apn);
        }
    }

    setProviderVerizon(provider) {
        let isProviderVerizon = false;
        if (provider === "Verizon" && this.state.selectedCountry == "United States") {
            isProviderVerizon = true;
        }
        this.props.callBackParent('isProviderVerizon', isProviderVerizon);
    }
    ...
    handleProviderSelect(e) {
        ...
        this.setProviderVerizon(e.target.value);
    }
    ...
    handleAdvancedOptions(e) {
        this.props.callBackParent(e.target.id, e.target.value);
    }
    ...
    renderProvider() {
        ...
        if (this.state.selectedCountry !== "other") {
            ...
            for (var i = 0; i < cellularData.length; i++) {
                rows.push(<option key={this.state.selectedCountry +"_"+cellularData[i]['provider']}
                    value={cellularData[i]['provider']}>{cellularData[i]['provider']}</option>);
            }
        } else {
    ...
    renderAPN() {
        ...
        if (this.state.selectedProvider === "Verizon") {
            rows.push(<option key="VZWINTERNET" value="">VZWINTERNET</option>);
            this.apnDefaultValue = "";
        }
        else {
            this.apnDefaultValue = "";
            if (this.state.selectedCountry !== "other" && this.state.selectedProvider !== "other") {
                ...
                if (this.state.selectedProvider === 0) {
                    ...
                    for (var j = 0; j < arrAPN.length; j++) {
                        rows.push(<option key={cellularData[0]['provider'] +"_"+arrAPN[j]}
                            value={arrAPN[j]}>{arrAPN[j]}</option>);
                    }
                } else {
                    for (var i = 0; i < cellularData.length; i++) {
                        if (cellularData[i]['provider'] === this.state.selectedProvider) {
                            ...
                            for (var j = 0; j < arrAPN.length; j++) {
                                rows.push(<option key={this.state.selectedProvider +"_"+arrAPN[j]}
                                    value={arrAPN[j]}>{arrAPN[j]}</option>);
                            }
                            break;
                        }
        ...
        rows.push(<option key={this.state.selectedCountry + this.state.selectedProvider + "_custom"} value="custom">
            Custom</option>);
        ...
        return (
            <div className="formRow">
                ...
                <div className="input-box">
                    <input type="text" placeholder="Enter your custom APN here" maxLength="63" id="cellularCustomAPN"
                        onChange={this.handleAPNSelect.bind(this)}/>
                </div>
            </div>
        );
    }
        ...
        return (
            <div>
                ...
                <form className="cellularConfigForm" id="cellularConfigForm" method='post'>
                    ...
                    {(this.state.selectedProvider === "Verizon") ?
                        null :
                        <div id="cellularAdvanced" className="expander">
                            <a id="AdvancedLink" onClick={this.toggle}
                                className={this.state.showAdvanced ? "expander-trigger" : "expander-trigger expander-hidden"}>Advanced</a>
